    ...
    localTimer.current = setInterval(() => {
      ...
      if (i >= plan.length) {
        ...
        setFiles([
          {
            path: "index.html",
            content: `<!doctype html>\n<html lang="en"><head><meta charset="utf-8"><title>${title}</title></head>\n<body style="font-family:system-ui;display:grid;place-items:center;min-height:100vh;margin:0;background:#0B0C10;color:#EEF0F5"><main style="text-align:center"><h1 style="color:#5E5CE6">${title}</h1><p>Built by Vibex.</p></main></body></html>\n`,
          },
        ]);
        setDone(true);
      ...
      >
        ...
        <section className={styles.convo}>
          ...
          <div className={styles.paneHead}>
            ...
            <span className={styles.paneHeadRight}>
              {!live && <span className={styles.simBadge}>simulated</span>}
              {/* In Code mode the chat is a slide-in drawer — this closes (slides out) the drawer. */}
              {canvasTab === "code" && (
                <button type="button" className={styles.convoClose} onClick={() => setChatHidden(true)} aria-label="Hide chat" title="Hide chat">
        ...
        <section className={styles.canvas} ref={canvasRef}>
          <div className={styles.paneHead}>
            ...
            <div className={styles.canvasCtrls}>
              {/* Preview mode: chat is a left column, hidden/shown here. Code mode uses the drawer's
                  own close button + the floating Chat button instead. */}
              {canvasTab !== "code" && (
          ...
          {/* Floating Chat button — slides the conversation drawer back in (Code mode only). */}
          {canvasTab === "code" && chatHidden && (
            <button type="button" className={styles.chatReopen} onClick={() => setChatHidden(false)} aria-label="Show chat" title="Show chat">
              <svg viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round" aria-hidden><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z" /></svg>
              Chat
            </button>
          )}
