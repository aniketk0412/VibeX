...
`;
  ...
  if (path.endsWith(".css")) {
    return `:root { --bg: #0B0C10; --fg: #EEF0F5; --accent: #5E5CE6; }
...
export async function* runEngine(
  ...
  let totalTokens = 0;
  let totalCost = 0;
  let win: WindowState = opts.startWindow ?? { kind: "FIVE_HOUR", used: 0, startedAt: Date.now() };
  const files: GenFile[] = [];
  ...
  if (!opts.signal?.aborted) {
    ...
    let cost = 0;
    try {
      if (!reviewerC.call) throw new Error("no model");
      const r = await reviewerC.call(reviewerSystem(), reviewerUser(spec, files), 400);
